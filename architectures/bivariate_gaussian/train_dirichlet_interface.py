...
k3_test_set = pd.read_csv('data/gaussian_mixture/eight_gaussians.csv', header=None).values
    ...
    with tf.Session() as sess:
        ...
        for iter_ in range(ITERS):
            ...
            if (np.mod(iter_, FREQ) == 0) or (iter_+1 == ITERS):
                ...
                z_values = sess.run(disc_real, feed_dict={real_data: points})
                z= z_values#  np.random.randn(128*128)
                z  = z.reshape((len(x), len(y))).transpose()
                plt.contour(x, y ,z)
                plt.colorbar()  # add color bar

                plt.scatter(train_set[:, 0], train_set[:, 1], c='orange', marker='+')
                plt.scatter(fake_samples[:, 0], fake_samples[:, 1], c='green', marker='*')

                fig = plt.gcf()
                fig.set_size_inches(18.5, 10.5)

                plt.savefig("Good.png", dpi =300)
