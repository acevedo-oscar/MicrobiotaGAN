...
+ This particular version of the GAN uses the  double of nodes in the second and third layers
"""
...
def train_gan(train_set, indices: List, samples_per_N:int, repetition_n:int, identifier:str,experiment_name:str,  batch_size: int =256, desired_epochs: int = 1000):
    ...
    def Generator_Softmax(n_samples,  name='gen'):
        ...
        with tf.variable_scope(name):
            ...
            output04 = tf_utils.linear(output03, 2*DIM, name='fc-4')
            output04 = tf_utils.relu(output04, name='relu-4')

            output05 = tf_utils.linear(output04, 3*DIM, name='fc-5')
            output05 = tf_utils.relu(output05, name='relu-5')
            
            output06 = tf_utils.linear(output05, GEN_DIM, name='fc-6')

            # Reminder: a logit can be modeled as a linear function of the predictors
            output05 = tf.nn.softmax(output06, name = 'softmax-1')
    ...
    def Discriminator(inputs, is_reuse=True, name='disc'):
        with tf.variable_scope(name, reuse=is_reuse):
            ...
            output06 = tf_utils.linear(output05, DIS_DIM, name='fc-6')
