...
GEN_DIM = 50  # output dimension of the generator
...
print("==> Loading CSV")
train_data =  pd.read_csv('data/abundances.csv',header=None  ).values # np.random.normal(mu, sigma, (n_train_samples,numero_especies))
...
processed_train_data, sample_mean, sample_std = utils.process_data(train_data)
...
with tf.Session() as sess:
    ...
    for iter_ in range(ITERS):
        ...
        if (np.mod(iter_, FREQ) == 0) or (iter_+1 == ITERS):
            ...
            print("Fake samples dimensions "+str(fake_samples.shape))

            # one_fake_sample = fake_samples[0,:]
            
            # print(one_fake_sample.shape)
            
            corrected_sample = utils.inverse_process_data(fake_samples, sample_mean, sample_std)
            
            print("Corrected samples dimensions "+str(corrected_sample.shape))
            
            print("===> One fake sample "+str(corrected_sample[0,:]))
            
            glv_error = utils.GLV_Model(corrected_sample[0,:], m_A, m_r)
            
            print("===> Sample GLV Error "+str(glv_error))


            df = pd.DataFrame(corrected_sample)
            with open('glv_data.csv', 'a') as f:
