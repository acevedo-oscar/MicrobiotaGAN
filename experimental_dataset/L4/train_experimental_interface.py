...
------------------------------------------------
"""
def train_gan(train_set, test_set,  repetition_n:int, experiment_name:str,
    batch_size: int = 64, desired_epochs: int = 2000, use_bot = False):
    """
    ...
    """
    my_ds = DataSetManager(train_set)

    samples_per_N = train_set.shape[0]


    # print("Set number of iterations to train\n")
    v5 = (desired_epochs*(train_set.shape[0]))//batch_size +1

    print("ITERS "+str(v5))
    print("SIZE "+str(train_set.shape))
    ...
    GEN_DIM = train_set.shape[1]  # output dimension of the generator
    ...
    with tf.Session() as sess:
        ...
        for iter_ in range(ITERS):
            ...
            if (np.mod(iter_, FREQ) == 0) or (iter_+1 == ITERS):
                ...
                # print(all( k > 0 for k in  fake_population))
                # r = fake_population.sum(axis=1) > 0
                # print(all(r))

                print(np.round(fake_population.sum(axis=1)))
        ...
        # Create gan samples
        n_samples = train_set.shape[0] #len(indices)
