...
k3_test_set = pd.read_csv('data/k1_dir/k1_test_set.csv', header=None) .values
...
------------------------------------------------
"""
def train_gan(train_set, indices: List, samples_per_N:int, repetition_n:int, identifier:str,experiment_name:str,  batch_size: int =256, desired_epochs: int = 2000):
    """
    ...
    with tf.Session() as sess:
        ...
        for iter_ in range(ITERS):
            ...
            if (np.mod(iter_, FREQ) == 0) or (iter_+1 == ITERS):
                ...
                fake_population = np.array([ sess.run(fake_data) for k in range(40)]).reshape(40*256,100)
