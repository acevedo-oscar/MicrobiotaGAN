...
This version of the GAN uses a sparsemax activation function rather than
a softmax in the last layer of the Generator
"""
...
# from tensorflow.contrib import sparsemax.sparsemax as sparsemax
...
def train_gan(train_set, indices: List, samples_per_N:int, repetition_n:int, identifier:str,experiment_name:str,  batch_size: int =256, desired_epochs: int = 1000):
    ...
    def Generator_Sparsemax(n_samples,  name='gen'):
        ...
        with tf.variable_scope(name):
            ...
            output05 = tf.contrib.sparsemax.sparsemax(output04, name = 'sparsemax-1')
    ...
    fake_data = Generator_Sparsemax(BATCH_SIZE)
    ...
    with tf.Session() as sess:
        ...
        #for iter_ in range(ITERS):
            ...
            if (np.mod(iter_, FREQ) == 0) or (iter_+1 == ITERS):
                ...
                # save_gen_samples(fake_data, disc_fake ,sess, storing_path, k) # fake_data = Generator_Sparsemax(BATCH_SIZE)
